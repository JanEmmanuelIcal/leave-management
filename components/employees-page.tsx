...
import { type Employee, getEmployees, addEmployee, deleteEmployee, updateEmployee } from "@/lib/storage"
import { useAuth } from "@/hooks/use-auth"
import { Trash2, Plus } from "lucide-react"
import AddEmployeeForm from "@/components/add-employee-form"

export default function EmployeesPageComponent() {
  ...
  const handleApprove = (id: string) => {
    ...
    const updated = updateEmployee(id, { status: "approved" })
    if (updated) {
  ...
  // If user is not admin, hide the full employees list and show only their own profile
  if (role !== "admin") {
    const my = employees.find((e) => e.id === currentEmployeeId)
    return (
          ...
          {my ? (
            <Card className="bg-slate-700 border-slate-600 text-white">
              <CardHeader>
                ...
                <p className="text-sm text-slate-300 mt-1">{my.email}</p>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    <p className="text-slate-400">Department</p>
                    <p className="font-semibold">{my.department}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-blue-300">{my.annualLeave} days</p>
                  </div>
                  <div>
                    <p className="text-slate-400">Sick Leave</p>
                    <p className="font-semibold text-green-300">{my.sickLeave} days</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-700 border-slate-600 text-white text-center py-8">
              <p className="text-slate-300">No profile available. If you registered, please wait for admin approval.</p>
            </Card>
          )}
          <p className="mt-4 text-sm text-slate-400">You don't have permission to view other employees. Admins can manage all employees.</p>
        </div>
      </main>
    )
  }
          ...
          <div className="grid gap-4">
            {filteredEmployees.map((emp) => (
              <Card key={emp.id} className="bg-slate-700 border-slate-600 text-white">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      {emp.status === "pending" && role === "admin" && (
                        <Button size="sm" onClick={() => handleApprove(emp.id)} className="bg-green-600 hover:bg-green-500">
                          Approve
                        </Button>
                      )}
                      <Button
                ...
                {emp.status === "pending" && (
                  <CardContent>
                    <p className="text-yellow-300">Status: Pending approval</p>
                  </CardContent>
                )}
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
